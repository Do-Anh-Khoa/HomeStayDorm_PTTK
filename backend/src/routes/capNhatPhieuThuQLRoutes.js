...
import {
  ...
  loadLichSuCapNhat,
  xacNhanThanhToan,
} from '../controllers/capNhatPhieuThuQLController.js'

const router = Router()

router.use((req, res, next) => {
  if (req.auth?.loai_nv && req.auth.loai_nv !== 'QL') {
    return res.status(403).json({ message: 'Chỉ Nhân viên Quản lý được truy cập chức năng này.' })
  }
  next()
})
