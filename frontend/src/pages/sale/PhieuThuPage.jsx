...
const API_BASE = '/cap-nhat-phieu-thu-sale'
...
const noiDungThanhToan = (pt) => {
  if (!pt) return ''
  if (pt.loaiPT === 'dat-coc') {
    return `Thanh toán phiếu thu đặt cọc ${pt.maPDC || pt.maLienKet || ''} - ${pt.tenKH || ''}`
  }
  return `Thanh toán phiếu thu hợp đồng ${pt.maHDT || pt.maLienKet || ''} - ${pt.tenKH || ''}`
}
  ...
  return (
    <>
      <PhieuThuTable
        ...
        description="Nhân viên Sale cập nhật trạng thái thanh toán cho phiếu thu đặt cọc và phiếu thu hợp đồng."
        rows={dsCanCapNhat}
        loading={loading}
...
function ThongTinPhieuThu({ pt }) {
  const maLienKetLabel = pt.loaiPT === 'dat-coc' ? 'Mã phiếu đặt cọc' : 'Mã hợp đồng'
  ...
  return (
    <section style={S.panel}>
      ...
      <div style={S.inputGroup}>
        <label style={S.fieldLabel} htmlFor="ngayThanhToan">Thời điểm thanh toán do nhân viên Sale ghi nhận</label>
        <input
          id="ngayThanhToan"
          type="datetime-local"
          step="1"
...
export default function CapNhatPhieuThuSalePage() {
  ...
  const handleXacNhanThanhToan = async () => {
    ...
    // 2. Kiểm tra so với thời điểm thanh toán cũ nếu có
    ...
    <section>
      {view === 'list' ? (
        <div style={S.titleWrap}>
          <PageTitle
            title={pageTitle}
            description="Nhân viên Sale xác nhận thanh toán cho phiếu thu đặt cọc và phiếu thu hợp đồng."
          />
        </div>
      ) : (
        <SubHeader
